...
def _install_browsers():
    ...
    print("\n[1/5] \U0001f4e6 Installing Playwright browsers...")
    _run([sys.executable, "-m", "playwright", "install", "chromium"])


def _build_exe():
    """Run PyInstaller to build the .exe."""
    print("\n[2/5] \U0001f3a8 Building with PyInstaller...")

    spec_file = PROJECT_ROOT / "build.spec"
    _run([sys.executable, "-m", "PyInstaller", str(spec_file)])
...
def _copy_browsers(dist_dir: Path):
    ...
    print("\n[3/5] \U0001f4c1 Copying Playwright browsers...")
...
def _copy_templates(dist_dir: Path):
    ...
    print("\n[4/5] \U0001f4c4 Copying template files...")
    templates = [
...
def _write_readme(dist_dir: Path):
    ...
    print("\n[5/5] \U0001f4dc Writing usage guide...")
    readme = dist_dir / "使用说明.txt"
    readme.write_text(
        "\U0001f37f B站收藏夹智能分类 - 使用说明\n"
        "=" * 42
        "\n\n"
        "  1. 双击运行 B首收藏夹分类.exe\n"
        "  2. 在弹出的浏览器中扫码登录 B站\n"
        ...
        "  • 若分类结果中 '其他'过多，可在 seed_mappings.json "
        "中添加 UP主映射\n\n"
        "\U0001f6d1 问题排查:\n"
